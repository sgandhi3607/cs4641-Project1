...
layer_1_size_list = range(10, 90, 10)

for activation in activation_functions:
    train_err = [0] * len(layer_1_size_list)
    test_err = [0] * len(layer_1_size_list)
    for i, o in enumerate(offsets):
        for layer_1_size in layer_1_size_list:
            print 'activation and layer_1_size: ' + activation + "  size: " + str(layer_1_size)
            ...
            mlp = MLPClassifier(activation=activation, alpha=0.0001, batch_size='auto', beta_1=0.9,
               beta_2=0.999, epsilon=1e-08,
               hidden_layer_sizes=(layer_1_size, 13, 13), learning_rate='constant',
               learning_rate_init=0.001, max_iter=500, momentum=0.9,
               nesterovs_momentum=True, random_state=None,
               shuffle=True, solver='adam', tol=0.0001, validation_fraction=0.1)
            print 'training',
    ...
    plt.figure()
    title = 'Census Income: Performance x Hidden Layer 1 Size using Activation ' + activation
    plt.title('\n'.join(wrap(title,60)))
    # plt.subplots_adjust(top=0.85)
    plt.plot(layer_1_size_list, test_err, '-', label='test error')
    plt.plot(layer_1_size_list, train_err, '-', label='train error')
    plt.legend()
    plt.xlabel('Hidden Layer 1 Size')
    plt.ylabel('Mean Square Error')
    filename = 'censusIncome_' + activation + '__varyingArchitecture_PerformancexTrainingSetSize.png'
    plt.savefig('plots/CensusIncome/NN/zeroes_and_one/varyingArchitecture/' + filename)
    print 'plot complete'
    ### ---
